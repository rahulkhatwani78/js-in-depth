# Week 6 Practical Task - TypeScript & PostgreSQL CRUD API

This project is a practical implementation of a RESTful API using Node.js, Express, TypeScript, and PostgreSQL. It demonstrates how to perform CRUD (Create, Read, Update, Delete) operations on a PostgreSQL database containing a `customers` table using modern ECMAScript Modules (ESM) and strongly-typed code.

## Features

- **TypeScript Support**: Fully strongly typed to help prevent runtime errors.
- **ES Modules (ESM)**: Built using modern `import` and `export` syntax.
...
- **tsx / nodemon**: For seamless TypeScript execution and auto-reloading during development

## Project Structure

```text
015-week6-practicalTask/
├── src/
│   ├── config/
│   │   └── connection.ts           # PostgreSQL connection pool setup
│   ├── controllers/
│   │   └── user.controller.ts      # Route handler logic for user actions
│   ├── data/
│   │   └── createCustomersTable.ts # Table init script with dummy data insertion
│   ├── middlewares/
│   │   └── index.ts                # Request logging and global error handler
│   ├── models/
│   │   └── user.model.ts           # Database queries interaction
│   ├── routes/
...
```
*(Make sure to adjust the key names based on what is used in `connection.ts`)*
